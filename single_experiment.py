...
"""
把所有factors和methods放在同一个函数结构里，需要添加则直接在其中添加声明，不直接放在主调用函数里修改
"""
#def methods_factors(image_crop:原图被切割成的子图, outer_list: 原图的标注文件中所标注的bndbox所在的外点列表, iteration: 迭代次数)
def methods_factors(img_name, image_crop, groundtruth_list):
    #所有的factors集合
    #factors = (gray_mean, contrast_mean_4neighbor, contrast_mean_8neighbor, saturation_mean)
    factors = (gray_mean, contrast_mean_4neighbor, contrast_mean_8neighbor, saturation_mean)
    factors_str = ("gray_mean", "contrast_mean_4neighbor", "contrast_mean_8neighbor", "saturation_mean")

    #三种计数方法的阈值比例集合,间隔0.01取值
    rate_of_extpnts = np.linspace(0.01, 0.2, 20, True)

    #计算外点的方法集合
    methods1 = (calculate_std_diff)
    methods2 = (borda_count, copeland_count, maximin_count)
    methods2_str = ("borda_count", "copeland_count", "maximin_count")
    ...
    for factor in range(len(factors)):
        ...
        for i in range(0, height, sub_size):
            for j in range(0, width, sub_size):
                ...
                #该ij下的子图数据值
                val = round(factors[factor](img_cropped, pixel_size))
                if val > max_val:
                    max_val = val
                fac_val_list[iterations].extend([iterations, val])
                iterations += 1
        for epsilon in range(1, zsRansac_epsilon_range):
            for threshold in range(1, zsRansac_threshold_range):
                threshold_list, std_var_list = ransac_threshold_slope0(list=fac_val_list, point_total=subimg_num, max_val=max_val, iters=1000, epsilon=epsilon, threshold=threshold)
                #for method in range(len(methods1)):
                predict_point_list = calculate_std_diff(threshold_list, std_var_list)#calcu_diff方法返回值为多个阈值下的外点矩阵
                len_of_pre = len(predict_point_list)
                for i in range(len_of_pre):
                    #评估矩阵，包含置信度、混淆矩阵参数
                    evaluation_matrix = evaluation_judge(groundtruth_point_list=groundtruth_list,
                                                         predict_point_list=predict_point_list[i])
                    #存储路径
                    csv_save_path = "E:\\LIC\\CV_detection\\experiment_records\\" \
                                    + "subimage_size_" + str(sub_size) \
                                    + "\\factor_" + factors_str[factor] \
                                    + "\\epsilon_" + str(epsilon) \
                                    + "\\ransac_threshold_" + str(threshold) \
                                    + "\\method_" + "calculate" \
                                    + "\\threshold_" +str(i) \
                                    + "\\confusion_matrix"
                    matrix_save_name = str(img_name) + "_outer_confusion_matrix.csv"
                    pre_point_name = str(img_name) + "_predict_point.csv"
                    save_csv_single(csv_save_path, matrix_save_name, evaluation_matrix)
                    print("\nData: subimage_size: " + str(sub_size) \
                          + ", factor: " + factors_str[factor] \
                          + ", epsilon: " + str(epsilon) \
                          + ", ransac_threshold: " + str(threshold) \
                          + ", method: calculate" \
                          + ", predict_threshold: " + str(i) \
                          + ", " + str(img_name) + " real outer confusion matrix" \
                          + ", has been saved successfully.")
                    save_csv_multi(csv_save_path, pre_point_name, predict_point_list[i])
                    print("\nData: subimage_size: " + str(sub_size) \
                                              + ", factor: " + factors_str[factor] \
                                              + ", epsilon: " + str(epsilon) \
                                              + ", ransac_threshold: " + str(threshold) \
                                              + ", method: calculate" \
                                              + ", predict_threshold: " + str(threshold_list[i]) \
                                              + ", " + str(img_name) + " predicted points matrix" \
                                              + " has been saved successfully.")

                for method in range(len(methods2)):
                    for rate_of_extpnt in rate_of_extpnts:
                        predict_point_list = methods2[method](rate_of_extpnt, threshold_list, std_var_list)#三种计数方法返回值为单个概率阈值下的外点矩阵
                        evaluation_matrix = evaluation_judge(groundtruth_point_list=groundtruth_list,
                        ...
                        csv_save_path = "E:\\LIC\\CV_detection\\experiment_records\\" \
                                        ...
                                        + "\\epsilon_" + str(epsilon) \
                                        + "\\ransac_threshold_" + str(threshold) \
                                        + "\\method_" + methods2_str[method] \
                                        + "\\rate_of_exterior_" + str(rate_of_extpnt) \
                                        + "\\confusion_matrix"
                        matrix_save_name = str(img_name) + "_outer_confusion_matrix.csv"
                        pre_point_name = str(img_name) + "_predict_point.csv"
                        save_csv_single(csv_save_path, matrix_save_name, evaluation_matrix)
                        print("\nData: subimage_size: " + str(sub_size) \
                              ...
                              + ", epsilon: " + str(epsilon) \
                              + ", ransac_threshold: " + str(threshold) \
                              + ", method: " + methods2_str[method] \
                              + ", rate of exterior points: " + str(rate_of_extpnt) \
                              + ", " + str(img_name) + "  real outer confusion matrix" \
                              + " has been saved successfully.")
                        save_csv_multi(csv_save_path, pre_point_name, predict_point_list)
                        print("\nData: subimage_size: " + str(sub_size) \
                              + ", factor: " + factors_str[factor] \
                              + ", epsilon: " + str(epsilon) \
                              + ", ransac_threshold: " + str(threshold) \
...
con4_cal_evaluation_all_matrix = [[] for i in range(img_num_total)]
con4_bo_evaluation_all_matrix = [[] for i in range(img_num_total)]
con4_co_evaluation_all_matrix = [[] for i in range(img_num_total)]
con4_ma_evaluation_all_matrix = [[] for i in range(img_num_total)]

#可以单一计算也可以整体计算，使用methods_factors函数，但是很慢
for img_num in tqdm(range(0, img_num_total-1), desc='Processing image ', ncols=200, leave=True):
    ...
    con4_fac_val_list = [[] for i in range(subimg_num)]  # 储存不同值的list
    iterations = 0
    con4_max_val = 0
    for i in range(0, height, sub_size):
        for j in range(0, width, sub_size):
            ...
            con4_val = contrast_mean_4neighbor(img_cropped, pixel_size)
            if con4_val > con4_max_val:
                con4_max_val = con4_val
            con4_fac_val_list[iterations].extend([iterations, con4_val])


            iterations += 1


#以下的部分都可以自行添加（不同factors和不同models计算出来的结果）
    con4_threshold_list, con4_std_var_list = ransac_threshold_slope0(list=con4_fac_val_list, point_total=subimg_num, max_val=con4_max_val, iters=1000, epsilon=epsilon, threshold=threshold)
    ...
    con4_bo_scorelist, con4_bo_predict_point_list = borda_count(rate_of_extpnt=0.2, std_list=con4_std_var_list)
    con4_co_scorelist, con4_co_predict_point_list = copeland_count(rate_of_extpnt=0.2, std_list=con4_std_var_list)  # calcu_diff方法返回值为多个阈值下的外点矩阵
    con4_ma_scorelist, con4_ma_predict_point_list = maximin_count(rate_of_extpnt=0.2, std_list=con4_std_var_list)
    # 评估矩阵，包含置信度、混淆矩阵参数
    ...
    con4_bo_evaluation_matrix = evaluation_judge(groundtruth_point_list=outer_list, predict_point_list=con4_bo_predict_point_list)
    con4_bo_evaluation_all_matrix[img_num].extend(con4_bo_evaluation_matrix)
    con4_co_evaluation_matrix = evaluation_judge(groundtruth_point_list=outer_list, predict_point_list=con4_co_predict_point_list)
    con4_co_evaluation_all_matrix[img_num].extend(con4_co_evaluation_matrix)
    con4_ma_evaluation_matrix = evaluation_judge(groundtruth_point_list=outer_list,predict_point_list=con4_ma_predict_point_list)
    con4_ma_evaluation_all_matrix[img_num].extend(con4_ma_evaluation_matrix)
